  ...
  return (
    <div className="fixed top-0 left-0 w-full border shadow-sm h-[55px] flex text-sky-900 bg-white px-2">
      ...
      <div className="max-w-screen-lg m-auto flex items-center w-full">
        ...
        <ul
          className={`sm:hidden md:items-center md:pb-0 p-6 absolute md:static bg-slate-500 md:z-auto z-[-1] top-14  w-full md:w-auto md:pl-0 pl-9 transition-all duration-300 ease-in ${
            open ? 'left-0' : 'left-[100%]'
          }`}
        >
          {links.map((link) => (
            <li key={link.name} className=" text-xl">
              <Link to={link.link} className="text-gray-800 hover:text-gray-400 duration-300">
                {link.name}
              </Link>
            </li>
          ))}
